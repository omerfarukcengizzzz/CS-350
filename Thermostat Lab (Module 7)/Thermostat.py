...
#    2          Added error handling for I2C sensor failures
#------------------------------------------------------------------
...
class ManagedDisplay():
    ...
    def cleanupDisplay(self):
        try:
            ...
            if DEBUG:
                print(f"Error clearing LCD during cleanup: {e}")
        
        # Clean up digital IO pins
        try:
            self.lcd_rs.deinit()
            self.lcd_en.deinit()
            self.lcd_d4.deinit()
            ...
            if DEBUG:
                print(f"Error cleaning up LCD GPIO pins: {e}")
    ...
    def updateScreen(self, message):
        try:
            ...
            if DEBUG:
                print(f"Error updating LCD screen: {e}")
...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## Last known good temperature reading for fallback
    ##
    lastGoodTemp = 72.0
    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            if temp < self.setPoint:
                # Temperature is below setpoint - fade red LED
                redLight.pulse()
            else:
                # Temperature is at or above setpoint - solid red LED
                redLight.on()
        elif self.current_state.id == "cool":
            if temp > self.setPoint:
                # Temperature is above setpoint - fade blue LED
                blueLight.pulse()
            else:
                # Temperature is at or below setpoint - solid blue LED
                blueLight.on()
    ...
    def run(self):
        ...
        myThread.daemon = True  # Make thread daemon so it dies when main program exits
        myThread.start()

    ##
    ## Get the temperature in Fahrenheit with error handling
    ##
    def getFahrenheit(self):
        try:
            t = thSensor.temperature
            # Update last good temperature reading
            fahrenheit_temp = (((9/5) * t) + 32)
            self.lastGoodTemp = fahrenheit_temp
            return fahrenheit_temp
        except OSError as e:
            if DEBUG:
                print(f"I2C Error reading temperature sensor: {e}")
                print(f"Using last good temperature: {self.lastGoodTemp}")
            # Return last known good temperature
            return self.lastGoodTemp
        except Exception as e:
            if DEBUG:
        ...
        while not self.endDisplay:
            try:
                ...
                if(altCounter < 6):
                    ...
                    lcd_line_2 = f"ST:{self.current_state.id} Set:{self.setPoint}F"
